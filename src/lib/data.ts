...
export const WEEKS: WeekBlock[] = [
  {
    ...
    tasks: [
      ...
      { title: 'CDN setup (CloudFront / Cloudflare)', area: 'Infra', responsible: 'mageshop', observation: 'Configurado junto com a instalação do Magento na W1. Cache de assets + imagens desde o início.' },
    ],
    milestonesAtEnd: [
  ...
  },
  {
    ...
    tasks: [
      ...
      { title: 'Export de clientes da VTEX (somente campos básicos)', area: 'Migração', responsible: 'client', observation: 'Apenas CPF, nome, sobrenome e e-mail. VTEX exporta muito lixo, então filtra só o essencial. Pedidos NÃO são importados. Senha: fluxo OTP no Increazy (mesmo padrão da Ajinomoto).' },
    ],
    milestonesAtEnd: [],
  },
  {
    ...
    tasks: [
      ...
      { title: 'Contrato de API ERP → Magento (cadastro e carga de produtos)', area: 'Migração', responsible: 'both', responsibility: 'ERP do cliente envia via API. Mageshop recebe.', observation: 'Produtos NÃO vêm da VTEX — planilhas da VTEX são caóticas de propósito. Cadastro e carga vêm do ERP pela API, mantendo os SKUs originais pra matching.' },
      { title: 'Import de produtos + variantes via API do ERP', area: 'Migração', responsible: 'mageshop', observation: '19k SKUs com Configurable Products nativos. SKUs preservados do ERP.' },
    ],
    milestonesAtEnd: [],
  },
  {
    ...
    tasks: [
      ...
      { title: 'Import de imagens de produto', area: 'Migração', responsible: 'mageshop', observation: 'Imagens vêm junto com os produtos via API do ERP (mesmo fluxo dos SKUs).' },
  ...
  },
  {
    ...
    tasks: [
      ...
      { title: 'Import de base de clientes (básico)', area: 'Migração', responsible: 'mageshop', observation: 'Só CPF, nome, sobrenome e e-mail do export VTEX. Login via OTP no Increazy (sem migrar senha).' },
  ...
  },
  {
    ...
    tasks: [
      ...
      { title: 'Sitemap XML + submissão ao Google', area: 'SEO', responsible: 'nerau', observation: 'Gerado e servido pelo frontend PWA (Increazy). Submetido via Search Console.' },
      { title: 'Robots.txt configurado', area: 'SEO', responsible: 'nerau', observation: 'Configurado no PWA Increazy. Bloqueia staging e áreas admin, libera o catálogo.' },
